	...
	if player_node.check_stealth(stealth_detection_multiplier): # Player script method
		...
		if health_component.is_damaged:
			...
			if parent_npc.is_angry:
				...
				parent_npc.say("start_trading")
				%TraderInventory.trade_target = player_node
				Logger.log(parent_npc.npc_name, "Предложил торговлю")
				await get_tree().create_timer(1.0).timeout
				request_state_change(statemachine_node.state.TRADE)
